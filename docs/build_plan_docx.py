# -*- coding: utf-8 -*-
"""将 系统功能提升实施方案.md 转换为 系统功能提升实施方案.docx（python-docx）。"""
...
MD_PATH = BASE / "系统功能提升实施方案.md"
OUT_PATH = BASE / "系统功能提升实施方案.docx"
...
def setup_header_footer(doc):
    ...
    run = hp.add_run("“潭合同”台账管理系统 · 系统功能提升实施方案")
...
def make_table(doc, rows, widths_cm):
    ...
    apply_geometry(table, widths_cm)
    return table
...
def add_image(doc, rel_path, alt):
    ...
    run = p.add_run()
    run.add_picture(str(img_path), width=Cm(15.2))
    cap = doc.add_paragraph()
    cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    cap.paragraph_format.space_after = Pt(12)
    cr = cap.add_run(f"图：{alt}")
    set_font(cr, size=9, color=MUTED_COLOR)
...
def parse_md(text):
    ...
    while i < len(lines):
        ...
        if line.startswith(">"):
            ...
            blocks.append(("meta" if not seen_rule else "callout", text))
            i += 1
            continue
...
def main():
    ...
    doc.core_properties.title = "“潭合同”台账管理系统 系统功能提升实施方案"
    doc.core_properties.author = "湘潭市机关事务管理局"

    numbered = {"num_id": None, "open": False}
    toc_skip = False
    meta_pending = []
    title_done = False
    toc_placeholder_done = False
    ...
    for kind, payload in blocks:
        ...
        if kind == "h":
            ...
            if text == "目录":
                ...
                p.paragraph_format.space_after = Pt(8)
                add_inline(p, "目  录", size=16, bold=True, color=TITLE_COLOR)
                if not toc_placeholder_done:
                    tp = doc.add_paragraph()
                    tr = tp.add_run("[[TOC]]")
                    set_font(tr, size=11)
                    toc_placeholder_done = True
                toc_skip = True
                continue
            toc_skip = False
            close_list()
            if level == 1:
                if not title_done:
                    ...
                    add_inline(tp, "“潭合同”台账管理系统", size=22, bold=True, color=TITLE_COLOR)
                    sp = doc.add_paragraph()
                    sp.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    sp.paragraph_format.space_after = Pt(10)
                    add_inline(sp, "系统功能提升实施方案", size=16, bold=True, color=H1_COLOR)
                    title_done = True
                    continue
                p = doc.add_paragraph(style="Heading 1")
                add_inline(p, text, size=16, bold=True, color=H1_COLOR)
            elif level == 2:
                p = doc.add_paragraph(style="Heading 1")
                add_inline(p, text, size=16, bold=True, color=H1_COLOR)
            elif level == 3:
                p = doc.add_paragraph(style="Heading 2")
                add_inline(p, text, size=13, bold=True, color=H2_COLOR)
            else:
                p = doc.add_paragraph(style="Heading 3")
                add_inline(p, text, size=12, bold=True, color=H3_COLOR)
            continue
        if kind == "meta":
            meta_pending.append(payload)
            if title_done:
                for line in meta_pending:
                    p = doc.add_paragraph()
                    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    p.paragraph_format.space_after = Pt(2)
                    add_inline(p, line, size=10.5, color=MUTED_COLOR)
                meta_pending = []
            continue
        if toc_skip:
            if kind in ("bullet", "para", "num"):
                continue
        if kind == "para":
            close_list()
            p = doc.add_paragraph()
        ...
        if kind == "image":
            close_list()
            add_image(doc, payload[0], payload[1])
            continue
        if kind == "table":
            close_list()
            rows = payload
